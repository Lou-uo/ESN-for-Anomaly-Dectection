...
from if_model import IFAnomalyDetector
...
def main():
    # 全局配置（与GRU/OCSVM保持一致，便于对比）
    global_config = {
        "K": 10,
        "n_runs": 3,  # 多次运行取均值
        "device": "cpu",  # IF仅支持CPU（但支持多核心加速）
        "datasets": [],
        "dataset_sigma": {
            "smd": 3.0,
            "smap_msl": 2.0,
        ...
        "if_params": {
            ...
            "contamination": 0.05,  # 异常比例（与数据污染率对齐）
            "max_samples": "auto",
            "max_features": 1.0,
            "bootstrap": False
        }
    ...
    print(f"🚀 孤立森林（Isolation Forest）异常检测模型训练器 | 使用设备：{global_config['device']}")
    print(
        f"⚙️  IF配置：n_estimators={global_config['if_params']['n_estimators']}, contamination={global_config['if_params']['contamination']}")
            ...
            for file_path in test_files:
                ...
                try:
                    file_generator = UnifiedTimeSeriesDatasetGenerator(
                        ...
                        anomaly_contamination=global_config["if_params"]["contamination"],
                        file_path=file_path
                    )
                ...
                for run_idx in range(global_config["n_runs"]):
                    ...
                    # 初始化IF检测器
                    detector = IFAnomalyDetector(
                        n_estimators=global_config["if_params"]["n_estimators"],
                        contamination=global_config["if_params"]["contamination"],
                        max_samples=global_config["if_params"]["max_samples"],
                        max_features=global_config["if_params"]["max_features"],
                        bootstrap=global_config["if_params"]["bootstrap"],
                        random_state=42 + run_idx
                    )

                    # 训练IF
                    print(f"📈 开始训练孤立森林 | 树数量：{detector.n_estimators}, 异常比例：{detector.contamination}")
                    ...
                    # 评估（使用统一的evaluate_model函数，与GRU/OCSVM对齐）
