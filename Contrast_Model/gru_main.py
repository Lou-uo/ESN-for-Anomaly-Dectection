...
# 导入GRU核心实现
from gru_model import LightweightGRU, GRUAnomalyDetector
...
def main():
    ...
    print(f"🚀 轻量化GRU异常检测模型训练器 | 使用设备：{global_config['device']}")
    print(f"⚙️  GRU基础配置：hidden_dim={global_config['hidden_dim']}, num_layers={global_config['num_layers']}")
    ...
    for dataset_name in global_config["datasets"]:
        ...
        if dataset_name == "smap_msl":
            ...
            test_files = [f for f in test_files if os.path.basename(f).startswith("M-3")]
            if len(test_files) == 0:
            ...
            for file_path in test_files:
                ...
                for run_idx in range(global_config["n_runs"]):
                    ...
                    print(f"✅ GRU模型初始化完成 | 可训练参数总数：{param_count:,}")
