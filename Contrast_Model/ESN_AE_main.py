...
from train_eval import set_graded_seed, evaluate_model, train_model
# 导入ESN_AE核心实现
from ESN_AE_model import LightweightESN_AE, ESNAnomalyDetector

# ========== 新增：导入标准VUS计算函数（和其他模型统一） ==========
...
def main():
    ...
    print(f"🚀 轻量化ESN异常检测模型训练器 | 使用设备：{global_config['device']}")
    print(f"⚙️  ESN_AE基础配置：hidden_dim={global_config['hidden_dim']}, num_layers={global_config['num_layers']}")
    ...
    for dataset_name in global_config["datasets"]:
        ...
        if dataset_name == "smap_msl":
            ...
            test_files = [f for f in test_files if os.path.basename(f).startswith("E-3")]
            if len(test_files) == 0:
            ...
            for file_path in test_files:
                ...
                for run_idx in range(global_config["n_runs"]):
                    ...
                    # 初始化ESN_AE检测器
                    detector = ESNAnomalyDetector(
                        input_dim=n_vars,
                        hidden_dim=global_config['hidden_dim'],
                        device=global_config["device"]
                    )
                    # 关键修复2：手动设置优化器（覆盖默认lr）
                    detector.optimizer = torch.optim.Adam(
                        detector.model.parameters(),
                        lr=global_config['lr'],
                        weight_decay=1e-6
                    )
                    param_count = sum(p.numel() for p in detector.model.parameters() if p.requires_grad)
                    print(f"✅ ESN_AE模型初始化完成 | 可训练参数总数：{param_count:,}")

                    # 训练 - 关键修复3：改用detector自身的train方法（避免直接调用有问题的train_model）
                    print(
                        f"📈 开始训练 | epochs={global_config['epochs']}, batch_size={global_config['batch_size']}, lr={global_config['lr']}")
                    loss_history = detector.train(
                        train_windows=train_windows,
                        epochs=global_config["epochs"],
                        batch_size=global_config["batch_size"],
                        verbose=False  # 关闭内部打印，统一外部日志
                    )
                    print(f"✅ 训练完成 | 最后一轮损失：{loss_history[-1]:.6f}")
